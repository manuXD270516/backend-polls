...
const { sendRecoverPasswordMailTo } = require('../core/services/mailer.service');
...
const authenticateUser = async (req, res) => {
  try {
    ...
    let userFind = await UsersRepository.getUserByUsernameAndType(Email, userType);
    if (userFind) {
      ...
      if (mathchPassword) {
        ...
        let userWithRoles = await UsersRepository.getUserWithActiveRoles(UserId);

        let authToken = jwtAuthorization.generateTokenKey(userWithRoles, userType);
        ...
        switch (parseInt(userType)) {
          case UserType.USER_POLLESTER:
			...
			let pollsterFind = await PollsterRepository.pollsterExists({Email});
			authenticateResponse = {...authenticateResponse, data: pollsterFind }
            break;
			case UserType.USER_SUPERVISOR:
				
				let supervisorFind = await SupervisorRepository.supervisorExists({Email});
				authenticateResponse = {...authenticateResponse, data: supervisorFind }
      ...
      statusCodeHttp = StatusCodeHTTP.UNAUTHORIZED_HTTP;
    }
}
...
const logoutUser = async (req, res) => {
  try {
    ...
    const response = await sequelizeInstance.transaction(async (t) => {
      let closeSession = await UsersSessionRepository.closeSession(tokenSession, userId);
    ...
    if (response) {
      ...
      return responseServer(res, entityUserName, StatusCodeDomain.RESOURCE_UPDATED_FAILURE);
    }
  } catch (error) {
