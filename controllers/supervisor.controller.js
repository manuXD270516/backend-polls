...
const { SupervisorRepository, UsersRepository } = require('../core/models/repositories');
...
const getAllSupervisors = async (req, res) => {
  try {
    let supervisors = mapperSupervisor(await SupervisorRepository.getAllSupervisors());
...
const registerSupervisor = async (req, res) => {
  try {
    ...
    let transaction = await sequelizeInstance.transaction(async (t) => {
      ...
      let { SupervisorId } = await SupervisorRepository.registerSupervisor(supervisorBody);

      // Register User
      await UsersRepository.registerUser({ ...User, SupervisorId });

      return { success: true, message: 'Supervisor registrado correctamente' };
    });
    let { success = false } = transaction;
    if (success) {
      return responseServer(res, transaction);
    }
    return responseServer(res, entitySupervisor, StatusCodeDomain.TRANSACTION_ERROR);
  } catch (error) {
    return responseServer(res, error, StatusCodeHTTP.INTERNAL_SERVER_ERROR_HTTP);
  }
};
